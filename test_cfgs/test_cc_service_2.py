...
if __name__ == '__main__':
    ...
    #im_names = im_names[:100];

    for im_name in im_names:
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('Demo for /{}'.format(im_name))
        bbox = detect_objects(sess, net, im_name)

        if bbox is None:
        	print "No Amazon Card Found"
        	#raise MyValidationError("No Amazon Card Found")
    	else:
    		print bbox
    		im_file = os.path.join(input_dir, im_name)
    		print im_file
	    	...
	    	cv2.imwrite(os.path.join(output_dir, im_name), corrected_image)
